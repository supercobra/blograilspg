...
if (ENV["LOGSTASH_UNICORN_URI"] || "").length > 0
  require_relative '../lib/bamzooka_logstash_logger'
  require_relative '../lib/unicorn_logstash_patch'
  logger BamzookaLogstashLogger.logger(uri: ENV['LOGSTASH_UNICORN_URI'], type: :unicorn)
end

bamzooka_path = File.expand_path(File.expand_path(File.dirname(__FILE__)) + "/../")

# tune down if not enough ram
worker_processes (ENV["UNICORN_WORKERS"] || 3).to_i

working_directory bamzooka_path

# listen "#{bamzooka_path}/tmp/sockets/unicorn.sock"
listen ENV["UNICORN_LISTENER"] || "#{(ENV["UNICORN_BIND_ALL"] ? "" : "127.0.0.1:")}#{(ENV["UNICORN_PORT"] || 3000).to_i}"

if !File.exist?("#{bamzooka_path}/tmp/pids")
  FileUtils.mkdir_p("#{bamzooka_path}/tmp/pids")
end

# feel free to point this anywhere accessible on the filesystem
pid (ENV["UNICORN_PID_PATH"] || "#{bamzooka_path}/tmp/pids/unicorn.pid")
...
if ENV["RAILS_ENV"] != "production"
  ...
  stderr_path "#{bamzooka_path}/log/unicorn.stderr.log"
  stdout_path "#{bamzooka_path}/log/unicorn.stdout.log"
  ...
  unless initialized
    ...
    if sidekiqs > 0
      ...
      Demon::Sidekiq.after_fork do
        BamzookaEvent.trigger(:sidekiq_fork_started)
      end
    ...
    if ENV['BAMZOOKA_ENABLE_EMAIL_SYNC_DEMON'] == 'true'
    ...
    BamzookaPluginRegistry.demon_processes.each do |demon_class|
    ...
    class ::Unicorn::HttpServer
      ...
      def check_sidekiq_heartbeat
        ...
        if @sidekiq_next_heartbeat_check < Time.now.to_i
          ...
          if sidekiq_rss > max_allowed_sidekiq_rss
            Rails.logger.warn("Sidekiq is consuming too much memory (using: %0.2fM) for '%s', restarting" % [(sidekiq_rss.to_f / 1.megabyte), ENV["BAMZOOKA_HOSTNAME"]])
            restart = true
          end
      ...
      def check_email_sync_heartbeat
        ...
        last_heartbeat_ago = Time.now.to_i - Bamzooka.redis.get(Demon::EmailSync::HEARTBEAT_KEY).to_i
        ...
        if email_sync_rss > max_allowed_email_sync_rss
          STDERR.puts("EmailSync is consuming too much memory (using: %0.2fM) for '%s', restarting" % [(email_sync_rss.to_f / 1.megabyte), ENV["BAMZOOKA_HOSTNAME"]])
          restart = true
        end
...
after_fork do |server, worker|
  BamzookaEvent.trigger(:web_fork_started)
  Bamzooka.after_fork
